...
pub fn part_two(input: &str, duration: u16) -> u16 {
    ...
    let mut distances: HashMap<&str, u16> = HashMap::new();
    ...
    for _ in 0..duration {
        for (reindeer, state) in &mut racers {
            match state {
                ...
                State::Flying { remaining_duration } => {
                    *distances.entry(reindeer.name).or_insert(0) += reindeer.flight_speed;
        ...
        let leading_distance = distances
            .values()
            .max()
            .expect("there should be at least one distance value");

        for leader in distances
            .iter()
            .filter(|(_, distance)| **distance == *leading_distance)
            .map(|(name, _)| name)
        {
            *scores.entry(leader).or_insert(0) += 1;
        }
    }
