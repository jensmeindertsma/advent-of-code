...
fn measure_monkey_business(input: &str) -> usize {
    ...
    for round in 1..=20 {
        ...
        for monkey_position in 0..monkeys.len() {
            ...
            if monkeys[monkey_position].items.is_empty() {
                println!("Monkey has no items!");
            }

            while !monkeys[monkey_position].items.is_empty() {
                let monkey = &mut monkeys[monkey_position];
                let item = &mut monkey.items[0];
                ...
                monkeys[monkey_position].activity += 1;
            }
        }
    }
